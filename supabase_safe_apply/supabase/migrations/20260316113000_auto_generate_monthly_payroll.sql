-- Auto-generate monthly payslips on the 3rd of every month.
-- Generates missing payslips from employee joining month up to previous month.
-- Uses salary details and joining date from Payroll page data (employee_details).
-- Includes only active employees with Annual CTC > 0.
...
BEGIN
  WITH employee_salary AS (
    ...
    WHERE COALESCE(
        NULLIF(ed.total_annual_ctc, 0),
        NULLIF(ed.annual_ctc, 0),
        0
      ) > 0
...
IS 'Runs monthly payroll generation for all employees with annual CTC > 0, creating missing payslips from joining month through previous month.';
...
BEGIN
  ...
  -- Run once at migration time to backfill immediately.
  PERFORM public.generate_monthly_payslips_on_third(CURRENT_DATE);
EXCEPTION
  WHEN undefined_table THEN
    RAISE NOTICE 'cron.job table not available; monthly payroll schedule was not created in this environment.';
END;
$$;
